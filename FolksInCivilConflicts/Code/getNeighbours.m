    ...
    for k1=x-vision:x+vision        %goes throw the column
        for k2=y-vision:y+vision    %goes throw the rows
            if(k1>0 && k2>0 && k1<width && k2<height)
                if((chooser==0 && world(k2,k1).person.number==0)||(chooser==1 && world(k2,k1).person.number>0))     
                    %if field is in the whished state (empty/occupied): wright it in the neighbours array and 
